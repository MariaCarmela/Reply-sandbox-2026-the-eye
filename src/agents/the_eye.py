...
# ---------------------------------------------------------------------------
# Manual ground-truth labels (from challenge personas)
# ---------------------------------------------------------------------------
...
MANUAL_LABELS_LEV2: Dict[str, int] = {
    "RMYCVSOK": 0,  # Craig Daniel – healthy student
    "BONIFJME": 0,  # Sarah Callahan – stable routine
    "QOHAQRQI": 1,  # Kelly Davis – isolation, poor sleep, headaches
    "UEAPYAVH": 0,  # Mira Budig – young and healthy
    "DQQTUJMK": 0,  # Valeska Kobelt – active, managed
    "ETGUMXJD": 0,  # Samantha Dobson – stable
    "OKJSZSSW": 0,  # Evamaria Junitz – very active, managed
    "JYEAPNDE": 0,  # Jesse Garcia – well-coordinated care
    "IGKMHDGI": 1,  # Joseph Klingelhofer – side pain, insomnia, fatigue
    "TGUUITBW": 0,  # Jessica Molina – very active
    "UGACLBDX": 0,  # Isabelle Collin – healthy
    "GGGZNWZD": 1,  # Luc Verdier – alcohol, chest tightness, withdrawal
    "FLDORAQE": 0,  # Laura Santana – healthy student
    "GPXWMDDO": 0,  # Eugenia Brichese – active business owner
    "KIJIPGYX": 0,  # Susanne Adolph – stable freelancer
}

MANUAL_LABELS_LEV3: Dict[str, int] = {
 "GKXTLRIZ": 0, # Arnulfo - active creator
 "NANSUMZD": 1, # Nicole Burton - isolation, sleep issues, declining
 "RNTNCKYE": 0, # Edoardo - very active student
 "QHIUGITL": 1, # Michael Blumel - homebound, multiple meds, alone
 "DPGYNMKN": 0, # Raymond - active restaurateur
 "VKEIFVZP": 0, # Baptist - disciplined consultant
 "YMVTHTGN": 0, # Olivier - stable routine
 "ERGZRVUY": 1, # Luc Antoine - confined, weight loss, isolated
 "LVCSDQET": 0, # Mariana - active delivery worker
 "ODOPLOUK": 0, # Ryan Evans - active entrepreneur
 "DBJFADVJ": 0, # Christine - active boutique owner
 "OKFRZTFR": 0, # Hartwig - physically active
 "KRCTQOSM": 0, # Hans Peter - robust and active
 "ZXSGVPZY": 0, # Toby - healthy student
 "UISLLTXM": 1, # Suzanne Bonnin - weight loss, isolated, declining
 "UPPIPXVV": 1, # Mariusz Flantz - fatigue, cognitive concerns, declining
 "WWVYDVEF": 0, # Nicole Joseph - active consultant
 "SBXIDGUU": 0, # Roger - active photographer
 "BMPOTIFN": 0, # Holly - active pub owner
 "NEIJPWOU": 0, # Maggie - active and healthy
 "OSSURNNV": 0, # Ryan Chambers - extremely active
 "TVTHMCWI": 0, # Jacob - structured routine
 "XDSXRGHX": 0, # Dylan - fit and energetic
 "WZSVQCDR": 0, # Eitel - very active student
 "XZXWWTMD": 0, # Lynda - active delivery worker
 "THBNCXVE": 0, # Valentin - active consultant
 "AXWRAMKB": 0, # Marcantonio - healthy digital nomad
 "JPTJKBYK": 0, # Peter - stable and calm
 "UPGGVIQC": 0, # Ira - active designer
 "KRPGSKCH": 0, # Kerry - very active PA
}
...
class DataAgent:
    ...
    def _assign_labels(self) -> None:
        """Assign binary labels from hardcoded manual maps."""
        hardcoded = {
            1: MANUAL_LABELS_LEV1,
            2: MANUAL_LABELS_LEV2,
            3: MANUAL_LABELS_LEV3,
        }
        if self.level in hardcoded:
            self.labels = dict(hardcoded[self.level])
            n_pos = sum(self.labels.values())
            self.logger.info(
                "  Labels assigned from hardcoded map (level %d): %d at-risk.",
                self.level, n_pos,
            )
        else:
            self.logger.warning("No labels defined for level %d — defaulting to 0.", self.level)
            self.labels = {cid: 0 for cid in self._get_all_citizen_ids()}
...
class TheEye:
    ...
    def run(self) -> Tuple[Path, pd.DataFrame]:
        ...
        # ----------------------------------------------------------------
        self.logger.info("[Stage 4/5] PredictionAgent — train & predict …")
        ...
        # Stage 5 : Output
        # ----------------------------------------------------------------
        self.logger.info("[Stage 5/5] OutputAgent — saving submission …")
